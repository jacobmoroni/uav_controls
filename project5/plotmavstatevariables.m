...
%     psi         = uu(9);             % yaw angle (degrees)
    ...
    Va          = uu(14);            % airspeed (m/s)
    alpha       = 180/pi*uu(15);     % angle of attack (degrees)
    beta        = 180/pi*uu(16);     % side slip angle (degrees)
    wn          = uu(17);            % wind in the North direction
    we          = uu(18);            % wind in the East direction
    wd          = uu(19);            % wind in the Down direction
    ...
    pn_hat      = uu(32);            % estimated North position (meters)
    pe_hat      = uu(33);            % estimated East position (meters)
    h_hat       = uu(34);            % estimated altitude (meters)
    Va_hat      = uu(35);            % estimated airspeed (meters/s)
    alpha_hat   = 180/pi*uu(36);     % estimated angle of attack (degrees)
    beta_hat    = 180/pi*uu(37);     % estimated side slip angle (degrees)
    phi_hat     = 180/pi*uu(38);     % estimated roll angle (degrees)   
    theta_hat   = 180/pi*uu(39);     % estimated pitch angle (degrees)
    chi_hat     = 180/pi*uu(40);     % estimated course (degrees)
    p_hat       = 180/pi*uu(41);     % estimated body angular rate along x-axis (degrees/s)
    q_hat       = 180/pi*uu(42);     % estimated body angular rate along y-axis (degrees/s)
    r_hat       = 180/pi*uu(43);     % estimated body angular rate along z-axis (degrees/s)
...
%    bz_hat      = uu(49);            % estimated z-gyro bias
    delta_e     = 180/pi*uu(51);     % elevator angle (degrees)
    delta_a     = 180/pi*uu(52);     % aileron angle (degrees)
    delta_r     = 180/pi*uu(53);     % rudder angle (degrees)
    delta_t     = uu(54);            % throttle setting (unitless)
